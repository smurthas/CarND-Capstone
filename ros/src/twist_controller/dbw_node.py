...
class DBWNode(object):
	...
	self.controller = Controller(wheel_base, steer_ratio, max_lat_accel, max_steer_angle, vehicle_mass, wheel_radius, decel_limit, brake_deadband)
	...
	if(msg.data != self.dbw_enabled):
	    self.controller.acc_pid_controller.reset()
	...
	last_time = rospy.get_rostime()
        while not rospy.is_shutdown():
            ...
            if self.dbw_enabled and self.velocity is not None and self.twist is not None:
                time = rospy.get_rostime()
	    	dt = time - last_time
		last_time = time
                throttle, brake, steer = self.controller.control(self.twist.twist.linear.x, self.twist.twist.angular.z, self.velocity.twist.linear.x, dt.to_sec())
